...
ret_t window_switch_open(void) {
  ...
  widget_child_on(win, "s1", EVT_VALUE_WILL_CHANGE,
                  on_switch_will_change, win);
  widget_child_on(win, "s1", EVT_VALUE_CHANGED,
                  on_switch_changed, win);
  
  widget_child_on(win, "s2", EVT_VALUE_WILL_CHANGE,
                  on_switch_will_change, win);
  widget_child_on(win, "s2", EVT_VALUE_CHANGED,
                  on_switch_changed, win);
  
  widget_child_on(win, "s3", EVT_VALUE_WILL_CHANGE,
                  on_switch_will_change, win);
  widget_child_on(win, "s3", EVT_VALUE_CHANGED,
                  on_switch_changed, win);
